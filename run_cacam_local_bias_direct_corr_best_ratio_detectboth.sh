...
for dataset in $DATASETS; do
  ...
  echo "[corr][direct][detectboth][ratio=$ratio] $dataset"
  "$PY" ./scripts/run_benchmark.py \
    ...
    --model-hyper-params "{\"seq_len\": 100, \"d_model\": 128, \"n_heads\": 4, \"dropout\": 0.1, \"train_epochs\": 10, \"batch_size\": 128, \"learning_rate\": 1e-4, \"lradj\": \"type1\", \"patience\": 3, \"pct_start\": 0.3, \"causal_method\": \"corr\", \"causal_max_lag\": 3, \"anomaly_ratio\": [$ratio], \"causal_bias_mode\": \"direct\"}" \
    --gpus 0 \
    --num-workers 1 \
    --timeout 60000 \
    --save-path "label/cacam_local_bias_ablation/corr/direct_best_ratio_detectboth"
done
