...
from ts_benchmark.baselines.catch.utils.tools import adjust_learning_rate
...
class CATCH:
    def __init__(self, **kwargs):
        ...
        self.device = get_torch_device(allow_mps=False)
        self.criterion = nn.MSELoss()
        self.auxi_loss = frequency_loss(self.config)
        self.seq_len = self.config.seq_len
        self.model_name = "CATCH"
    ...
    def _normalize_observability_config(self):
        ...
        if debug:
            debug_num_runs = int(get_cfg_value(config, "debug_num_runs", 1))
            num_runs = max(1, debug_num_runs)
        ...
        early_stopping = as_bool(get_cfg_value(config, "early_stopping", True))
        set_cfg_value(config, "early_stopping", early_stopping)
    ...
    def _prepare_experiment_paths(self):
        ...
        write_json(self.experiment_paths.config_path, config_payload)
    ...
    def detect_validate(self, valid_data_loader, criterion, max_batches=None):
        total_loss = []
        total_samples = 0
        start = time.time()
        self.model.eval()
        ...
        total_loss = np.mean(total_loss) if total_loss else float("nan")
        speed = total_samples / elapsed if elapsed > 0 else None
        latency = elapsed / total_samples * 1000 if total_samples > 0 else None
        self.model.train()
        return {
    ...
    def _run_single_experiment(self, run_id, run_seed, train_points, valid_points, paths):
        config = self.config
        logger = build_logger(f"catch.run{run_id}.{id(self)}", paths.detail_log_path(run_id))
        ...
        try:
            self.model = CATCHModel(config).to(self.device)
            ...
            for epoch in range(config.num_epochs):
                ...
                adjust_learning_rate(self.optimizer, scheduler, epoch + 1, config, printout=False)
    ...
    def _train_one_epoch(self, epoch, run_id, train_steps, max_train_batches=None):
        train_loss = []
        grad_norms = []
        total_samples = 0
        ...
        step = max(1, min(int(len(self.train_data_loader) / 10), 100))
        batches = self._limited_batches(self.train_data_loader, max_train_batches)
        progress = tqdm(
            batches,
            total=train_steps,
            desc=f"CATCH run{run_id} epoch {epoch + 1}/{self.config.num_epochs}",
            unit="batch",
            dynamic_ncols=True,
            leave=False,
        )
        for i, (input, target) in progress:
    ...
    def _log_training_summary(self, logger, summary, history):
        ...
        logger.info(
            f"Early Stop     : {'Yes' if summary.get('early_stopped') else 'No'}"
        )
    ...
    def _log_test_results(self, run_id, ratio, metrics, fit_time_sec, inference_time_sec):
        logger = build_logger(
            f"catch.test.run{run_id}.{id(self)}",
    ...
    def detect_score(self, test: pd.DataFrame) -> np.ndarray:
        ...
        config = self.config
        max_eval_batches = (
            int(get_cfg_value(config, "debug_max_val_batches", 2))
            if as_bool(get_cfg_value(config, "debug", False))
            else None
        )
        ...
        test_labels = []
        with torch.no_grad():
            for i, (batch_x, batch_y) in self._limited_batches(self.thre_loader, max_eval_batches):
    ...
    def detect_label(self, test: pd.DataFrame) -> np.ndarray:
        ...
        self.freq_anomaly_criterion = frequency_criterion(config)
        with torch.no_grad():
            for i, (batch_x, batch_y) in self._limited_batches(self.train_data_loader, max_eval_batches):
                batch_x = batch_x.float().to(self.device)
                # reconstruction
                outputs, _, _ = self.model(batch_x)
                # criterion
        ...
        test_labels = []
        with torch.no_grad():
            for i, (batch_x, batch_y) in self._limited_batches(self.test_data_loader, max_eval_batches):
                batch_x = batch_x.float().to(self.device)
                # reconstruction
                outputs, _, _ = self.model(batch_x)
                # criterion
