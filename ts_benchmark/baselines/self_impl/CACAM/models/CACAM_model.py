...
class CausalConstraintAttention(nn.Module):
    ...
    def forward(self, x, causal_weight):
        ...
        # Apply causal constraint as additive attention bias when enabled.
...
class Basic_CACAM(nn.Module):
    def __init__(self, configs):
        ...
        self.use_causal_weight = _to_bool(getattr(configs, "use_causal_weight", True))
        self.causal_max_lag = getattr(configs, "causal_max_lag", 3)
        self.causal_pc_alpha = getattr(configs, "causal_pc_alpha", 0.05)
        self.causal_static = _to_bool(getattr(configs, "causal_static", False))
    ...
    def set_static_causal_weight(self, weight):
        ...
        if weight.dim() == 3:
            weight = weight[0]
        self.static_causal_weight = self._normalize_causal_weight(weight.detach())

    def compute_causal_weight(self, x):
        if self.causal_static and self.static_causal_weight is not None:
            return self.static_causal_weight.to(
    ...
    def forward(self, x):
        ...
        causal_weight = self.compute_causal_weight(x) if self.use_causal_weight else None
