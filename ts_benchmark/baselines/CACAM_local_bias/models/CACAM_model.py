...
def _parse_optional_threshold(value):
    if value is None:
        return None
    if isinstance(value, str):
        stripped = value.strip()
        if stripped == "" or stripped.lower() in {"none", "null", "false", "off"}:
            return None
        value = stripped
...
class CausalConstraintAttention(nn.Module):
    def __init__(self, d_model, n_heads=1, dropout=0.1, causal_bias_mode="log"):
        super(CausalConstraintAttention, self).__init__()
        self.d_model = d_model
        self.n_heads = n_heads
        self.causal_bias_mode = str(causal_bias_mode).lower()
    ...
    def forward(self, x, causal_weight):
        ...
        # Apply causal constraint as additive attention bias.
        causal_bias = causal_weight.unsqueeze(1)
        if self.causal_bias_mode == "log":
            causal_bias = torch.log(causal_bias + 1e-8)
        elif self.causal_bias_mode in ("direct", "hardcode", "hardcoded", "raw"):
            pass
        else:
            raise ValueError(
                f"Unsupported causal_bias_mode={self.causal_bias_mode!r}. "
                "Use one of: log, direct."
            )
...
class Basic_CACAM(nn.Module):
    def __init__(self, configs):
        ...
        self.causal_bias_mode = getattr(configs, "causal_bias_mode", "log")
        self.causal_method = getattr(configs, "causal_method", "correlation")
        self.causal_binary_threshold = _parse_optional_threshold(
            getattr(configs, "causal_binary_threshold", None)
        )
        self.causal_max_lag = getattr(configs, "causal_max_lag", 3)
        self.causal_pc_alpha = getattr(configs, "causal_pc_alpha", 0.05)
        self.causal_static = getattr(configs, "causal_static", False)
        self.static_causal_weight = None
        
        # Feature projection: mapping sequence to hidden representation
        ...
        self.causal_attn = CausalConstraintAttention(
            ...
            causal_bias_mode=self.causal_bias_mode,
        )
    ...
    def _finalize_causal_weight(self, weight):
        weight = torch.nan_to_num(weight, nan=0.0, posinf=0.0, neginf=0.0)
        weight = torch.clamp(weight, min=0.0)
        if self.causal_binary_threshold is not None:
            weight = (weight >= self.causal_binary_threshold).to(weight.dtype)
            eye = torch.eye(weight.shape[-1], device=weight.device, dtype=weight.dtype)
            if weight.dim() == 3:
                eye = eye.unsqueeze(0)
            weight = torch.maximum(weight, eye)
            return weight
        return self._normalize_causal_weight(weight)

    def set_static_causal_weight(self, weight):
        if weight is None:
            self.static_causal_weight = None
            return
        if weight.dim() == 3:
            weight = weight[0]
        self.static_causal_weight = self._finalize_causal_weight(weight.detach())

    def compute_causal_weight(self, x):
        if self.causal_static and self.static_causal_weight is not None:
            return self.static_causal_weight.to(device=x.device, dtype=x.dtype).unsqueeze(0).expand(x.shape[0], -1, -1)
    ...
    def compute_causal_weight_pcmci(self, x):
        ...
        causal_weight = self._finalize_causal_weight(causal_matrix).unsqueeze(0).expand(B, -1, -1)
    ...
    def compute_causal_weight_correlation(self, x):
        ...
        return self._finalize_causal_weight(causal_weight)
    ...
    def compute_causal_weight_partial_correlation(self, x):
        ...
        return self._finalize_causal_weight(partial_corr)
    ...
    def compute_causal_weight_granger(self, x):
        ...
        return self._finalize_causal_weight(causal_matrix).unsqueeze(0).expand(B, -1, -1)
    ...
    def forward(self, x):
        ...
        # 2. Project sequence to hidden dimension for each channel
        # Transpose to [B, C, T] to learn channel interactions
        x_t = x.transpose(1, 2)
        h = self.feature_proj(x_t) # [B, C, d_model]
        
        # 3. Apply Causal Constraint Attention
        h_attn, attn_weights = self.causal_attn(h, causal_weight)
        
        # 4. Residual connection & Reconstruct
