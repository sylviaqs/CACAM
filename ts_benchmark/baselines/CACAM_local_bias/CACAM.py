...
from ts_benchmark.baselines.device import get_torch_device as get_available_device
...
class CACAM:
    def __init__(self, **kwargs):
        ...
        precision = str(getattr(self.config, "precision", "fp32")).lower()
    ...
    def _to_model_input(self, batch):
    ...
    def detect_validate(self, valid_data_loader):
        ...
        with torch.no_grad():
            for i, (input, _) in enumerate(valid_data_loader):
                input = self._to_model_input(input)
                with self._autocast():
    ...
    def _maybe_precompute_static_causal_weight(self, train_data_scaled: pd.DataFrame):
        ...
        if method not in {"corr", "correlation", "pearson", "granger", "granger_causality", "pcmci"}:
            return
        print(
        ...
        with torch.no_grad():
            if method in {"corr", "correlation", "pearson"}:
                weight = self.model.compute_causal_weight_correlation(x)
            elif method in {"granger", "granger_causality"}:
    ...
    def fit(self, train_data: pd.DataFrame, valid_data: pd.DataFrame = None):
        ...
        if valid_data is not None:
            if isinstance(valid_data, tuple):
                valid_data = valid_data[
                    0
                ]  # handle split_before tuple return if applicable
        ...
        for epoch in range(self.config.train_epochs):
            ...
            print(
                f"Epoch: {epoch + 1}, Steps: {train_steps} | Train Loss: {train_loss_avg:.7f} Vali Loss: {vali_loss:.7f}"
            )
    ...
    def detect_score(self, test: pd.DataFrame) -> np.ndarray:
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y) in enumerate(test_loader):
                batch_x = self._to_model_input(batch_x)
                with self._autocast():
                    reconstructed, _ = self.model(batch_x)
                    # Same final score as mean over C after concatenation, but
                    # avoids storing [num_windows, seq_len, num_channels] on RAM.
                    score = ((reconstructed - batch_x) ** 2).mean(dim=-1)

                # Anomaly score based on reconstruction error [B, T].
                score = score.detach().float().cpu().numpy()
                attens_energy.append(score)

        test_energy = np.concatenate(attens_energy, axis=0).reshape(-1)  # [nb*t]
    ...
    def detect_label(self, test: pd.DataFrame) -> np.ndarray:
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y) in enumerate(self.train_data_loader):
                ...
                with self._autocast():
                    reconstructed, _ = self.model(batch_x)
                    error = ((reconstructed - batch_x) ** 2).mean(dim=-1)
                train_energy.append(error.detach().float().cpu().numpy())
