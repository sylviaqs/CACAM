...
def save_log(
    result_df: pd.DataFrame, save_path, file_prefix: str, compress_method: str = None
) -> str:
    """
    Save log data.
    ...
    dataset_name = "multi"
    file_names = result_df.get("file_name")
    if file_names is not None:
        unique_file_names = [name for name in file_names.dropna().unique() if name]
        if len(unique_file_names) == 1:
            dataset_name = unique_file_names[0]

    record_filename = build_output_file_stem(file_prefix, dataset_name)
