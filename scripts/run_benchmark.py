...
def build_model_config(args: argparse.Namespace, config_data: Dict) -> Dict:
    ...
    for key in (
        ...
        if value is not None:
            observability_overrides[key] = value
    if args.early_stopping is not None:
        observability_overrides["early_stopping"] = bool(args.early_stopping)

    for adapter, model_name, model_hyper_params in zip(
        args.adapter, args.model_name, args.model_hyper_params
    ):
        parsed_model_hyper_params = (
            json.loads(model_hyper_params) if model_hyper_params is not None else {}
        )
        parsed_model_hyper_params.update(observability_overrides)
        model_config["models"].append(
            {
                "adapter": adapter,
                "model_name": model_name,
                "model_hyper_params": parsed_model_hyper_params,
            }
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=[0, 1],
        default=None,
        help="Enable fast debug training for models that support observability debug mode",
    )
    parser.add_argument(
        "--num-runs",
        type=int,
        default=None,
        help="Number of repeated training runs for models that support observability",
    )
    parser.add_argument(
        "--debug-num-runs",
        type=int,
        default=None,
        help="Number of runs used when --debug 1 is enabled",
    )
    parser.add_argument(
        "--debug-max-train-batches",
        type=int,
        default=None,
        help="Maximum train batches per epoch in debug mode",
    )
    parser.add_argument(
        "--debug-max-val-batches",
        type=int,
        default=None,
        help="Maximum validation/test batches in debug mode",
    )
    parser.add_argument(
        "--print-freq",
        type=int,
        default=None,
        help="Epoch interval for structured training progress logs",
    )
    parser.add_argument(
        "--early-stopping",
        type=int,
        choices=[0, 1],
        default=None,
        help="Enable or disable early stopping for models that support observability",
    )
    parser.add_argument(
        "--patience",
        type=int,
        default=None,
        help="Early stopping patience for models that support observability",
    )
    parser.add_argument(
        "--min-delta",
        type=float,
        default=None,
        help="Minimum validation improvement required for best checkpoint updates",
    )
